...
class MJPEGStreamReader:
    """
    Reads MJPEG stream from ESP32-CAM and provides frames.
    Runs in a background thread.
    """

    def __init__(self, url: str, on_frame: Callable[[bytes], None],
            ...
            while self._running:
                try:
                    ...
                    while True:
                        ...
                        if self._running and len(frame_data) > 100:
...
class CameraDiscovery:
    """Auto-discover ESP32-CAM devices on the network."""
    ...
    @staticmethod
    def scan_subnet(base_ip: str, on_found: Callable[[str], None],
                    on_complete: Callable[[], None],
                    on_progress: Callable[[int], None] = None) -> threading.Thread:
        """
        Scan local subnet for ESP32-CAM devices.
        Returns the thread for cancellation if needed.
        """
        def scan():
            ...
            for i in range(1, 255):
                ...
                ip = f"{subnet}.{i}"
                if CameraDiscovery._check_camera(ip):
                    found.append(ip)
                    on_found(ip)
    ...
    @staticmethod
    def _check_camera(ip: str, timeout: float = 0.3) -> bool:
        """Check if an IP has an ESP32-CAM stream."""
        try:
            sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
