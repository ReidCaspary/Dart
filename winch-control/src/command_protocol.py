...
class WinchStatus:
    ...
    max_jog_rps: float = 10.0
    max_move_rps: float = 7.5
    raw_response: str = ""

    @property
    def position_revolutions(self) -> float:
        """Convert position to revolutions (assuming 4000 steps/rev)."""
        return self.position / 4000.0
...
class ResponseParser:
    ...
    def parse_status(cls, response: str) -> Optional[WinchStatus]:
        ...
        max_jog_rps = float(vjog_str) if vjog_str else 10.0
        max_move_rps = float(vmove_str) if vmove_str else 7.5
